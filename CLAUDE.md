...
`skins.js` is a classic script loaded in `index.html` **before** `game.js`. It defines the `SKINS` registry (`retro`, `neon`, `pastel`, `pixel`), each with its own 8-color palette (indices 1-8, same order as piece types I/O/T/S/Z/J/L/Tuerca) and a `drawCell(context, x, y, colorIndex, size, alpha, overrideColor)` renderer. `game.js`'s `drawBlock()` is a thin delegator that calls `currentSkin().drawCell(...)`; `draw()`, `drawNext()`, and `drawPowerupBorder()` are unchanged and keep working identically across skins, including power-up tinting via `overrideColor`. `drawGrid()` prefers `currentSkin().gridColor` when a skin defines one, otherwise falls back to the `--grid-line` CSS var (theme system, unrelated). The active skin is persisted in `localStorage['tetris-skin']` and applied via `initSkin()` (called once at startup, right before `initTheme(); init();`), which also wires the `#skin-select` dropdown and toggles a `skin-*` class on `<html>`. The skin system is orthogonal to the light/dark theme system (`data-theme` attribute) — the two do not interact.
